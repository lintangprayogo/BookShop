...
<div class="col-md-8">
  ...
  <form enctype="multipart/form-data" class="bg-white shadow-sm p-3" action="{{route('users.update',[$user->id])}}" method="POST">
    ...
    <input class="form-control" type="text" id="name" name="name" class="form-control" placeholder="Full Name" value="{{$user->name}}">

    <label for="username">Username</label>
    <input class="form-control" type="text" id="username" name="username" class="form-control" placeholder="username" value="{{$user->username}}">
    ...
    Current avatar: <br>
    @if($user->avatar)
    <img src="{{asset('storage/'.$user->avatar)}}" width="120px" height="200px" />
    <br>
    @else
    No avatar
    @endif
